...
function DoAnscToFillDiv(sender, e, tbid, fk_mapExt) {
    ...
    if (myKeyCode == 38) {
        ...
        if (e != oldValue) {
            ...
            $.ajax({
                type: "get",
                url: "/WF/CCForm/HanderMapExt.ashx",
                data: json_data,
...
function ReturnValCCFormPopVal(ctrl, fk_mapExt, refEnPK) {
    ...
    url = '/WF/CCForm/FrmPopVal.aspx?FK_MapExt=' + fk_mapExt + '&RefPK=' + refEnPK + '&CtrlVal=' + ctrl.value;
...
function DDLFullCtrl(e, ddlChild, fk_mapExt) {
    ...
    $.ajax({
        type: "get",
        url: "/WF/CCForm/HanderMapExt.ashx?KVs=" + kvs,
        data: json_data,
