...
namespace BP.DA
{
    ...
    public class DBAccess
    {
        ...
        {
            int i = BP.DA.Ext.CRC32Helper.GetCRC32(Guid.NewGuid().ToString());
            if (i <= 0)
                i = -i;
            return i;
        }
	...
	public class DBAccessOfOracle
	{
		...
		public static float RunSQLReturnFloatVal(string sql )
		{
			...
			if (obj.ToString()=="System.DBNull")
				throw new Exception("@执行方法DBAccessOfOracle.RunSQLReturnFloatVal(sql)错误,运行的结果是null.请检查sql="+sql);
			...
			{
				throw new Exception("@执行方法DBAccessOfOracle.RunSQLReturnFloatVal(sql)错误,运行的结果是["+obj.ToString()+"].不能向float 转换,请检查sql="+sql);
			}
		}
		/// <summary>
		/// 运行sql 返回float
		...
		public static float RunSQLReturnFloatVal(string sql, float isNullAsVal)
		{
			...
			{
				throw new Exception("@执行方法DBAccessOfOracle.RunSQLReturnFloatVal(sql,isNullAsVal)错误,运行的结果是["+obj+"].不能向float 转换,请检查sql="+sql);
			}
		}
		/// <summary>
		...
		{			
			object obj=DBAccessOfOracle.RunSQLReturnVal( sql );
			if (obj==null)
				throw new Exception("@执行方法DBAccessOfOracle.RunSQLReturnDecimalVal()错误,运行的结果是null.请检查sql="+sql);
			try				
			{
				return decimal.Parse(obj.ToString());
			}
			...
			{
				throw new Exception("@执行方法DBAccessOfOracle.RunSQLReturnDecimalVal()错误,运行的结果是["+obj+"].不能向decimal 转换,请检查sql="+sql);
			}
		}
		/// <summary>
		...
		public static decimal RunSQLReturnDecimalVal(string sql, decimal isNullAsVal )
		{
			...
			{
				throw new Exception("@执行方法DBAccessOfOracle.RunSQLReturnDecimalVal(sql,isNullAsVal)错误,运行的结果是["+obj+"].不能向float 转换,请检查sql="+sql);
			}
		}
		#endregion 执行SQL ，返回首行首列
