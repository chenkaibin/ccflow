...
namespace BP.WF
{
    ...
    public class ClearRepLineLab : Method
    {
        ...
        public override object Do()
        {
            ...
            {
                sql = "DELETE " + item.EnMap.PhysicsTable + " WHERE FK_MapData='" + item.FK_MapData + "' AND X=" + item.X + " AND Y=" + item.Y + " and x1=" + item.X1 + " and x2=" + item.X2 + " and y1=" + item.Y1 + " and y2=" + item.Y2;
                DBAccess.RunSQL(sql);
            ...
            {
                sql = "DELETE " + item.EnMap.PhysicsTable + " WHERE FK_MapData='" + item.FK_MapData + "' and x=" + item.X + " and y=" + item.Y + " and Text='" + item.Text + "'";
                DBAccess.RunSQL(sql);
                item.MyPK = BP.DA.DBAccess.GenerOIDByGUID().ToString();
                item.Insert();
            }
